...
export const getNavigationByRole = (role?: string, isResponsable?: boolean, isJuryPresident?: boolean): NavigationConfig => {
  switch (role) {
    case 'admin':
      return {
        main: [
          ...
          { title: 'Signature', href: '/signature', icon: Signature },
        ]
    ...
    case 'enseignant': {
      ...
      // Add Signature menu item only if teacher is jury president
      if (isJuryPresident) {
