...
import type { Theme, JuryResponse, SubmitChoicesJuryResponse } from "@/types/juryTypes"
import { juryService } from "@/services/api"
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog"
import { fetchOptions } from "@/functions/fetchOptions"
import type { OptionItem } from "@/types/options"

interface FicheJuryProps {
  juryService: {
    ...
    ) => Promise<SubmitChoicesJuryResponse>
  }
}

const FicheJuryModern: React.FC<FicheJuryProps> = () => {
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        if (response.success) {
          setOptions(response.options.map((opt: OptionItem) => opt.option))
        }
      } catch (err) {
  ...
  useEffect(() => {
    const loadJuryData = async () => {
      try {
        ...
        const response = await juryService.getJuryData()
        console.log("Jury data response:", response)

        if (response.success) {
          setAvailableList(response.data.available_themes)
          setRankedList(response.data.current_choices || [])
        } else {
          setLoadingError(response.message || "Erreur lors du chargement des données")
        }
      } catch (error) {
  ...
  const getItemKey = (item: Theme) => `${item.id}-jury`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true)
      const response = await juryService.submitChoices(rankedThemes)
      console.log("Submit choices response:", response)

      if (response.success) {
        toast.success("Vos préférences de jury ont été enregistrées avec succès")
      } else {
        toast.error(response.message || "Une erreur est survenue lors de l'enregistrement")
        setValidationError(response.message || "Une erreur est survenue lors de l'enregistrement")
      }
    } catch (error: any) {
      if (error.response?.status === 422) {
        const validationErrors = error.response.data.errors
        const errorMessages = Object.values(validationErrors).flat().join(", ")
        const errorMsg = `Erreurs de validation: ${errorMessages}`
        toast.error(errorMsg)
        setValidationError(errorMsg)
      } else {
        const errorMsg = "Une erreur est survenue lors de l'enregistrement"
        toast.error(errorMsg)
        setValidationError(errorMsg)
      }
    ...
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 sm:px-6 py-8">
        <div className="text-center space-y-2 mb-8 lg:mb-12">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Participation au Jury PFE</h1>
          <p className="text-muted-foreground">
            {isMobile
              ? "Tapez sur les projets pour les ajouter à vos préférences de jury. Organisez-les par ordre de priorité."
              : "Glissez-déposez les sujets de PFE pour indiquer vos préférences de participation au jury. Organisez-les par ordre de préférence."}
          </p>
        </div>
            ...
            <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-4 lg:px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <h2 className="text-lg lg:text-xl font-semibold text-white">PFE Disponibles pour Jury</h2>
                </div>
                <select
                  value={filter}
                  ...
                  >
                    <div className="p-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {item.groupe?.moyenne && (
                          <span className={getBadgeStyle("moyenne")}>{item.groupe.moyenne}</span>
                        )}
                      </div>
                      {item.encadrant && (
                        <div className="mt-2 text-xs text-gray-500">
                          Encadrant: {item.encadrant}
                          {item.co_encadrant && ` • Co-encadrant: ${item.co_encadrant}`}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          >
            ...
            <div className="border-t bg-gray-50 px-4 lg:px-6 py-4">
              <button
                ...
                disabled={rankedList.length === 0 || isLoading}
              ...
              {rankedList.length === 0 && (
                <p className="text-center text-sm text-gray-500 mt-2">Sélectionnez au moins un projet pour continuer</p>
              )}
            </div>
          </div>
        </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    {touchDraggedItem.groupe?.moyenne && (
                      <span className={getBadgeStyle("moyenne")}>{touchDraggedItem.groupe.moyenne}</span>
                    )}
                  </div>
                </div>
              </div>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {selectedItem && (
                <>
                  ...
                  {selectedItem.groupe?.moyenne && (
                    <span className={getBadgeStyle("moyenne")}>{selectedItem.groupe.moyenne}</span>
                  )}
                </>
              )}
            </div>
          ...
          <div className="space-y-4">
            ...
            {selectedItem?.encadrant && (
              <div>
                <h4 className="font-semibold text-gray-900 mb-2">Encadrant</h4>
                <p className="text-gray-700">{selectedItem.encadrant}</p>
              </div>
            )}
            {selectedItem?.co_encadrant && (
              <div>
                <h4 className="font-semibold text-gray-900 mb-2">Co-encadrant</h4>
                <p className="text-gray-700">{selectedItem.co_encadrant}</p>
              </div>
            )}
...
export default FicheJuryModern
