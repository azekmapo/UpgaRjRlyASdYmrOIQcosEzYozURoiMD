...
import ChangePasswordEnseignantDialog from "@/components/custom/ChangePasswordEnseignantDialog"
...
const ProfilEnseignant: React.FC = () => {
  const [profileData, setProfileData] = useState<EnseignantProfile | null>(null)
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [responsableOption, setResponsableOption] = useState<string>("")
  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await profileService.getProfileEnseignant()

      if (response.success) {
        setProfileData(response.data)

        if (response.data.is_responsable) {
          try {
            const optionsResponse = await fetchOptions()
            if (optionsResponse.success) {
              const userOption = optionsResponse.options.find((option) => option.responsable?.id === response.data.id)

              if (userOption) {
                setResponsableOption(userOption.option)
              } else {
                setResponsableOption("Aucune option assignée")
              }
            }
          } catch (optionsError) {
            setResponsableOption("Erreur de chargement")
          }
        }
      } else {
        setProfileData(null)
    ...
    <div className="p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        <div className="text-center space-y-2 mb-8 md:mb-12">
          ...
          <p className="text-muted-foreground text-sm md:text-base">Consultez et gérez vos informations personnelles et professionnelles</p>
        </div>
        ...
        <div className="md:bg-white md:rounded-xl md:shadow-lg md:border md:border-gray-200 md:p-8">
          ...
          <form className="space-y-4 md:space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-6">
              <div>
                <label htmlFor="grade" className="block text-xs md:text-sm font-semibold text-gray-700 mb-1 md:mb-2">
                  Grade
                </label>
                <input
                  type="text"
              ...
              <div>
                <label htmlFor="date_recrutement" className="block text-xs md:text-sm font-semibold text-gray-700 mb-1 md:mb-2">
                  Date de Recrutement
                </label>
                <input
                  type="text"
                  id="date_recrutement"
                  value={new Date(profileData.date_recrutement).toLocaleDateString("fr-FR")}
                  className="w-full px-3 py-2 md:px-4 md:py-3 text-sm md:text-base bg-gray-50 border-2 border-gray-200 rounded-lg text-gray-900 font-medium focus:bg-white transition-all duration-200"
                  readOnly
                />
              </div>
            ...
            <div className={`grid gap-3 md:gap-6 ${profileData.is_responsable ? 'grid-cols-2' : 'grid-cols-1'}`}>
              <div>
              ...
              {profileData.is_responsable && (
                <div>
                  <label htmlFor="responsable" className="block text-xs md:text-sm font-semibold text-gray-700 mb-1 md:mb-2">
                    Option Responsable
                  </label>
                  <input
                    type="text"
                    id="responsable"
                    value={responsableOption || "Chargement..."}
                    className="w-full px-3 py-2 md:px-4 md:py-3 text-sm md:text-base bg-gray-50 border-2 border-gray-200 rounded-lg text-gray-900 font-medium focus:bg-white transition-all duration-200"
                    readOnly
                  />
                </div>
          ...
          <div className="mt-6 md:mt-8 text-center">
            <ChangePasswordEnseignantDialog>
